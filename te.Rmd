---
title: "2026 NFL Draft TE Grades"
...
blocking <- read.csv("data/offense_blocking.csv") %>% select(player_id, grades_run_block, grades_pass_block) # rb grade, pb grade
```

# Combine Datasets and Pull Names
```{r}
te_values <- tes %>% left_join(receiving, by = c("id" = "player_id")) %>% drop_na()
te_values <- te_values %>% left_join(blocking, by = c("id" = "player_id")) %>% drop_na()
te_values <- te_values %>% 
