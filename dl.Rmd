---
title: "2026 NFL Draft DL Grades"
...
# Load ED Players
```{r}
dls <- read.csv("data/players.csv") %>% filter(position == "DI")
```

# Load Necessary CSV Files
```{r}
defense <- read.csv("data/defense_summary.csv") %>% select(player_id, missed_tackle_rate, grades_pass_rush_defense, grades_run_defense) # miss %, prsh_grade, rdef_grade
...
# Combine Datasets
```{r}
dl_values <- dls %>% left_join(defense, by = c("id" = "player_id")) %>% drop_na()
dl_values <- dl_values %>% left_join(prsh_defense, by = c("id" = "player_id")) %>% drop_na()
dl_values <- dl_values %>% left_join(run_defense, by = c("id" = "player_id")) %>% drop_na()
dl_values <- dl_values %>% 
...
get_dl_ratings <- function(input_df) {
  df_dl_copy <- input_df %>% mutate(
    ...
    prp = round(pmax(pmin((prp-3)*1.25, 5), 0), 2), # 3-7, mean 5, worth 5
