---
title: "2026 NFL Draft LB Grades"
...
```{r}
defense <- read.csv("data/defense_summary.csv") %>% select(player_id, grades_run_defense, grades_pass_rush_defense, grades_coverage_defense, missed_tackle_rate, qb_rating_against) # rdef grade, prsh grade, cov grade, miss %, pass_rtg
run_defense <- read.csv("data/run_defense_summary.csv") %>% select(player_id, stop_percent) # run stop %
```

# Combine Datasets
```{r}
lb_values <- lbs %>% left_join(defense, by = c("id" = "player_id")) %>% drop_na()
lb_values <- lb_values %>% left_join(run_defense, by = c("id" = "player_id")) %>% drop_na()
lb_values <- lb_values %>% 
