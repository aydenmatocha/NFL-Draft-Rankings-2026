---
title: "2026 NFL Draft WR Grades"
...
```{r}
receiving <- read.csv("data/receiving_summary.csv") %>% select(player_id, grades_pass_route, yprr, drop_rate, contested_catch_rate) # rec grade, yprr, drop%, contested catch %
```

# Combine Datasets and Pull Names
```{r}
wr_values <- wrs %>% left_join(receiving, by = c("id" = "player_id")) %>% drop_na()
wr_values <- wr_values %>% 
...
get_wr_ratings <- function(input_df) {
  df_wr_copy <- input_df %>% mutate(
    ...
    drop_rate = round(pmax(pmin(((100-drop_rate)-90) * 1.67, 10), 0), 2), # 10-4, mean 7
