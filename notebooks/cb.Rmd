---
title: "2026 NFL Draft CB Grades"
...
```{r}
defense <- read.csv("data/defense_summary.csv") %>% select(player_id, missed_tackle_rate, qb_rating_against, catch_rate) # miss %, pass_rtg, comp %
...
# Combine Datasets
```{r}
cb_values <- cbs %>% left_join(defense, by = c("id" = "player_id")) %>% drop_na()
cb_values <- cb_values %>% left_join(cov_defense, by = c("id" = "player_id")) %>% drop_na()
cb_values <- cb_values %>% 
